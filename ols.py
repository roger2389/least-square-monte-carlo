...
def ols_lsmc(S, K, T, r, sigma, M, N, option='put', seed=123):
    ...
    for t in range(N-1, 0, -1):
        ...
        # Calculate continuation values using the OLS model
        C = ols_model.predict(X)
...
mc_std = []
for n in M:
    m, s = ols_lsmc(S, K, T, r, sigma, n, N, option)
    mc_mean.append(m)
    mc_std.append(s)
...
plt.title(f'S={S}, K={K}, T={T}, r={r}, $\sigma$={sigma}, option={option}, ols', fontsize=14)
